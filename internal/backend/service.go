...
func (s *Service) CompareService(envName, serviceName, version, serviceDefinition interface{}, parsedProvisioningConfig interface{}, configStoreNamespace *string) (interface{}, error) {
	client := newApiClient()
	data := map[string]interface{}{
		"serviceDefinition":    serviceDefinition,
...
func (s *Service) ScalingServiceConsent(serviceName string, data interface{}) (service.ScalingConsentResponse, error) {
	...
	response := client.actionWithRetry(path.Join(serviceEntity, serviceName)+"/scalingconsent", "GET", data)
	response.Process(true)
